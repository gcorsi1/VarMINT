...
F = interiorResidual(
    u_mid, p, v, q, rho, mu, mesh, u_t=u_t, Dt=Dt, C_I=Constant(6.0 * (k ** 4)), dx=dx
)
...
# fix pressure value at a point
corner_str = "near(x[0], 2.2) && near(x[1], 0.0)"
bcs = [
    DirichletBC(V.sub(0), Constant((0.0, 0.0)), boundaries, obstacle),
    DirichletBC(V.sub(1), Constant(0.0), corner_str, "pointwise"),
]

# assign the boundary conditions weakly
F += weakDirichletBC(
    u,
    p,
    v,
